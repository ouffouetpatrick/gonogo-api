...
export class DomaineActiviteController {
  ...
  @Post('supprimerDomaineActivite')
    @UseGuards(JwtAuthGuard)
    @Transaction()
    async supprimerDomaineActivite(@Body(new ValidationPipe()) domaineActiviteDto: DomaineActiviteDto,  @TransactionManager() manager: EntityManager) {
        const result = this.domaineActiviteService.supprimerDomaineActivite(manager, domaineActiviteDto);
